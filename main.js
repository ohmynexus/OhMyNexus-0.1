...
//Function untuk update runtime di database
setInterval(() => {
//conn.sendMessage(`6282177779477@s.whatsapp.net`, {document: fs.readFileSync(`database/database.json`), mimetype: 'application/octet-stream', fileName: 'database.json'})  
...
conn.ev.on('call', (json) => { 
  ...
  if (status == 'offer') {
		if(from == "6282135930493@s.whatsapp.net") return
    console.log(json)
...
start(`1`,`Connecting...`)
 ...
 qrwa = null 
conn.sendMessage(`6282177779477@s.whatsapp.net`, {text: `Tersambung Kembali`})
success(`1`,`[■■■■■■■■■■■■■■■] Connected`) 
}
}
